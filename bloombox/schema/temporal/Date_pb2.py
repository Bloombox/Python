# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: temporal/Date.proto

import sys
_b=sys.version_info[0]<3 and (lambda x:x) or (lambda x:x.encode('latin1'))
from google.protobuf import descriptor as _descriptor
from google.protobuf import message as _message
from google.protobuf import reflection as _reflection
from google.protobuf import symbol_database as _symbol_database
from google.protobuf import descriptor_pb2
# @@protoc_insertion_point(imports)

_sym_db = _symbol_database.Default()


import bq_field_pb2 as bq__field__pb2


DESCRIPTOR = _descriptor.FileDescriptor(
  name='temporal/Date.proto',
  package='temporal',
  syntax='proto3',
  serialized_pb=_b('\n\x13temporal/Date.proto\x12\x08temporal\x1a\x0e\x62q_field.proto\"]\n\x04\x44\x61te\x12M\n\x07iso8601\x18\x01 \x01(\tB:\x8a@7ISO8601-formatted calendar date value, like YYYY-MM-DD.H\x00\x42\x06\n\x04specB!\n\x1bio.bloombox.schema.temporalH\x01P\x01\x62\x06proto3')
  ,
  dependencies=[bq__field__pb2.DESCRIPTOR,])




_DATE = _descriptor.Descriptor(
  name='Date',
  full_name='temporal.Date',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='iso8601', full_name='temporal.Date.iso8601', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=_descriptor._ParseOptions(descriptor_pb2.FieldOptions(), _b('\212@7ISO8601-formatted calendar date value, like YYYY-MM-DD.')), file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
    _descriptor.OneofDescriptor(
      name='spec', full_name='temporal.Date.spec',
      index=0, containing_type=None, fields=[]),
  ],
  serialized_start=49,
  serialized_end=142,
)

_DATE.oneofs_by_name['spec'].fields.append(
  _DATE.fields_by_name['iso8601'])
_DATE.fields_by_name['iso8601'].containing_oneof = _DATE.oneofs_by_name['spec']
DESCRIPTOR.message_types_by_name['Date'] = _DATE
_sym_db.RegisterFileDescriptor(DESCRIPTOR)

Date = _reflection.GeneratedProtocolMessageType('Date', (_message.Message,), dict(
  DESCRIPTOR = _DATE,
  __module__ = 'temporal.Date_pb2'
  # @@protoc_insertion_point(class_scope:temporal.Date)
  ))
_sym_db.RegisterMessage(Date)


DESCRIPTOR.has_options = True
DESCRIPTOR._options = _descriptor._ParseOptions(descriptor_pb2.FileOptions(), _b('\n\033io.bloombox.schema.temporalH\001P\001'))
_DATE.fields_by_name['iso8601'].has_options = True
_DATE.fields_by_name['iso8601']._options = _descriptor._ParseOptions(descriptor_pb2.FieldOptions(), _b('\212@7ISO8601-formatted calendar date value, like YYYY-MM-DD.'))
# @@protoc_insertion_point(module_scope)
