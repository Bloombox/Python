# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: oauth/Client.proto

import sys
_b=sys.version_info[0]<3 and (lambda x:x) or (lambda x:x.encode('latin1'))
from google.protobuf import descriptor as _descriptor
from google.protobuf import message as _message
from google.protobuf import reflection as _reflection
from google.protobuf import symbol_database as _symbol_database
from google.protobuf import descriptor_pb2
# @@protoc_insertion_point(imports)

_sym_db = _symbol_database.Default()




DESCRIPTOR = _descriptor.FileDescriptor(
  name='oauth/Client.proto',
  package='oauth',
  syntax='proto3',
  serialized_pb=_b('\n\x12oauth/Client.proto\x12\x05oauth\"2\n\x06\x43lient\x12\x11\n\tclient_id\x18\x01 \x01(\t\x12\x15\n\rclient_secret\x18\x02 \x01(\t\"\x16\n\x08\x43lientID\x12\n\n\x02id\x18\x01 \x01(\t\"\x1e\n\x0c\x43lientSecret\x12\x0e\n\x06secret\x18\x01 \x01(\tB\x1e\n\x18io.bloombox.schema.oauthH\x01P\x01\x62\x06proto3')
)




_CLIENT = _descriptor.Descriptor(
  name='Client',
  full_name='oauth.Client',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='client_id', full_name='oauth.Client.client_id', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='client_secret', full_name='oauth.Client.client_secret', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=29,
  serialized_end=79,
)


_CLIENTID = _descriptor.Descriptor(
  name='ClientID',
  full_name='oauth.ClientID',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='id', full_name='oauth.ClientID.id', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=81,
  serialized_end=103,
)


_CLIENTSECRET = _descriptor.Descriptor(
  name='ClientSecret',
  full_name='oauth.ClientSecret',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='secret', full_name='oauth.ClientSecret.secret', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=105,
  serialized_end=135,
)

DESCRIPTOR.message_types_by_name['Client'] = _CLIENT
DESCRIPTOR.message_types_by_name['ClientID'] = _CLIENTID
DESCRIPTOR.message_types_by_name['ClientSecret'] = _CLIENTSECRET
_sym_db.RegisterFileDescriptor(DESCRIPTOR)

Client = _reflection.GeneratedProtocolMessageType('Client', (_message.Message,), dict(
  DESCRIPTOR = _CLIENT,
  __module__ = 'oauth.Client_pb2'
  # @@protoc_insertion_point(class_scope:oauth.Client)
  ))
_sym_db.RegisterMessage(Client)

ClientID = _reflection.GeneratedProtocolMessageType('ClientID', (_message.Message,), dict(
  DESCRIPTOR = _CLIENTID,
  __module__ = 'oauth.Client_pb2'
  # @@protoc_insertion_point(class_scope:oauth.ClientID)
  ))
_sym_db.RegisterMessage(ClientID)

ClientSecret = _reflection.GeneratedProtocolMessageType('ClientSecret', (_message.Message,), dict(
  DESCRIPTOR = _CLIENTSECRET,
  __module__ = 'oauth.Client_pb2'
  # @@protoc_insertion_point(class_scope:oauth.ClientSecret)
  ))
_sym_db.RegisterMessage(ClientSecret)


DESCRIPTOR.has_options = True
DESCRIPTOR._options = _descriptor._ParseOptions(descriptor_pb2.FileOptions(), _b('\n\030io.bloombox.schema.oauthH\001P\001'))
# @@protoc_insertion_point(module_scope)
